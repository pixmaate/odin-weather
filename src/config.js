...
class CurrentWeather {
    ...
    getNextDay(celsiusFarenheit) {
        fetch(`https://api.weatherapi.com/v1/forecast.json?key=${APIKEY}&q=${this.city}&days=2&aqi=no`, {mode: 'cors'})
        .then((response) => response.json())
        .then((response) => {
            if (celsiusFarenheit === 'C') {
...
class CurrentWeatherDOM {
    ...
    buildCondition() {
        ...
        weatherConditionDescription.style.cssText = 'font-size:1.3rem;font-weight:600;';
